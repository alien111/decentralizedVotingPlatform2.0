...
validatorAddress = ["127.0.0.1:5010", "127.0.0.1:5011"]
publicKey = {}

for i in range(len(validatorAddress)):
	answer = requests.get(f'http://{validatorAddress[i]}/getPublicKey')
	if (answer.status_code == 200):
		publicKey[validatorAddress[i]] = tuple(answer.json()['publicKey'])
	else:
		raise RuntimeError('Something went wrong while getting validator`s public key')

validatorPublicKeyFromLastSession = []
...
try:
	...
	for line in tmp:
		...
		local = (int(parts[0][1:]), int(parts[1][:len(parts[1]) - 1]))
		validatorPublicKeyFromLastSession.append(local)
...
if (validatorPublicKeyFromLastSession != [(10, 1)]):
	for i in validatorPublicKeyFromLastSession:
		tmp.write(str(i) + '\n')

for i in publicKey:
	tmp.write(str(publicKey[i]) + '\n')
tmp.close()
...
def verifySignature():
	...
	if (hashFromData == dataUnderSignature and (publicKey_ in validatorPublicKeyFromLastSession or publicKey_ in publicKey.values())):
...
def getResultsFromLastSession():
	...
	tmp = values['publicKey']
	for i in tmp:
		publicKeyFromLastSession.append(tuple(i))

	if (list(set(publicKeyFromLastSession) & set(validatorPublicKeyFromLastSession)) == [] and publicKeyFromLastSession != list(publicKey.values())):
		answer = {'message' : 'Key set from last session is invalid'}
		...
		for i in range(len(blockData)):
			if (len(blockData[i]) == 1 and type(blockData[i]) == list):						# [f'Poll {activePolls[i][0]} is finished']
				...
				if (blockData[i][1] not in finishedPolls):
					...
					for key in range(len(publicKeyFromLastSession)):
						hashFromData, dataUnderSignature = verifySignatureInnerFunction(data, signature, publicKeyFromLastSession[key])
			...
			if (len(blockData[i]) == 1 and type(blockData[i]) == list):						# [f'Poll {activePolls[i][0]} is finished']
				...
				if (blockData[i][1] not in finishedPolls):
					...
					for key in publicKey.keys():
						hashFromData, dataUnderSignature = verifySignatureInnerFunction(data, signature, publicKey[key])
