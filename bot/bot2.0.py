...
def selectValidatorNode():
	global validatorAddress
	global publicKey

	while True:
		if (validatorAddress == []):
			data = 'All validator nodes are unavailable!'
			bot.send_message(97781168, data)
			raise RuntimeError('All validator nodes are unavailable!')

		node = secrets.choice(validatorAddress)

		if (ping(node)):
			return node, publicKey[node]
		else:
			validatorAddress.remove(node)
			data = 'Validator node ' + node + ' is unavailable!'
			bot.send_message(97781168, data)
...
def backupChain(node):
	...
	validatorNode, publicKey = selectValidatorNode()
	...
	answer = requests.post(f'http://{validatorNode}/signData', json=json_data)
	if (answer.status_code == 201):
...
if ('--backupChain' in sys.argv):
	...
	for file in backupChains:
		if (file[5:len(file) - 4] in localNodes):
			...
			if (publicKeyFromLastSession not in validatorPublicKeyFromLastSession):
	...
	json_data = {'publicKey': validatorPublicKeyFromLastSession}
